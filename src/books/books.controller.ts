...
export class BookController {
    ...
    @Post()
    @UseInterceptors(FileInterceptor('image', { dest: './uploads/books' }))
    async createBook(@UploadedFile() image, @Body() createBookInput: CreateBookInput) {
        try {
            const data = { path: 'uploads/books/' + Date.now() + '_' + image.originalname };
            fs.renameSync('uploads/books/' + image.filename, data.path)
